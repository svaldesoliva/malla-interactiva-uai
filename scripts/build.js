...
function build(mode = "development") {
    ...
    // Create date.txt with commit info
    try {
        const commitTimestamp = `${execSync('git log -1 --format="%ct"').toString().trim()  }000`;
        const commitName = execSync('git log -1 --format="%s"').toString().trim();
        fs.writeFileSync(path.join(PUBLIC, "date.txt"), `${commitTimestamp}\n${commitName}`);
    } catch (e) {
        fs.writeFileSync(path.join(PUBLIC, "date.txt"), `${Date.now().toString()  }\nLocal Build`);
    }
